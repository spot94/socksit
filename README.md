...
(OpenWrt + Podkop/sing-box, а также mihomo/clash), то для доменов из своих списков он
...
**Причина:** SocksIt поднимает TUN и ставит маршрут по умолчанию `0.0.0.0/0` с метрикой 0,
поэтому перехватывает и пакеты к `198.18.x.x`. Они уходят в туннель вместо роутера,
sing-box на роутере их не видит — соединение мертво.

**Решение — уже включено по умолчанию.** SocksIt держит список **bypass-подсетей**, которые
исключаются из туннеля целиком: для них ставятся более специфичные маршруты через
физический шлюз, и они выигрывают у `0.0.0.0/0` по правилу
longest-prefix-match (независимо от метрики). По умолчанию в списке:

`198.18.0.0/15` (роутерный FakeIP), `127.0.0.0/8`, `169.254.0.0/16`, `224.0.0.0/4`,
`255.255.255.255/32`.

Диапазоны RFC 1918 (`10/8`, `172.16/12`, `192.168/16`) и так не проксируются — они идут
напрямую через правило приватных адресов и «прямые подсети».

Свой собственный FakeIP-диапазон SocksIt держит в `240.0.0.0/15` (Class E,
зарезервирован и не встречается в интернете) — именно чтобы не конфликтовать с роутером.

Настройка — ключ `bypass_cidrs` (см. таблицу ниже) или из CLI:

```bash
socksit config bypass list
```

Добавить свою подсеть, убрать лишнюю, вернуть значения по умолчанию:

```bash
socksit config bypass add 100.64.0.0/10
```
...
| `bypass_cidrs` | список CIDR | `198.18.0.0/15`, `127.0.0.0/8`, `169.254.0.0/16`, `224.0.0.0/4`, `255.255.255.255/32` | подсети, **исключаемые из туннеля целиком** (маршрут через физический шлюз). Нужны для роутерного FakeIP — см. раздел выше. Ключ отсутствует = значения по умолчанию; `bypass_cidrs: []` = отключить их |
...
| `dns.fakeip_v4` | CIDR | `240.0.0.0/15` | внутренний диапазон fake-ip (Class E — не пересекается с роутерным FakeIP; менять не нужно). Не должен попадать в `bypass_cidrs` |
