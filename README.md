...
| `dns.direct_domains` | список суффиксов | `.local`, `.lan`, `.internal`, `.home.arpa` | суффиксы, которым fake-ip **не** выдаётся: их настоящий адрес определяет маршрутизацию. Добавьте сюда корпоративный суффикс, если внутренние узлы адресуются по имени |
...
> по приложению. В обычном режиме (список = что проксировать) это ничего не меняет:
> остальное идёт напрямую, и настоящий адрес движок определяет сам. Но если вы проксируете
> **весь** трафик (`proxy_all`) или пользуетесь списком-исключением, внутренние узлы,
> к которым обращаются по имени, уйдут в прокси — перечислите их суффикс в
> `dns.direct_domains`. Обращения по IP работают как раньше в любом режиме.
