...
- **Приоритет:** вшитый пресет (Схема A) важнее файла-соседа `socksit.preset.yaml`
  (Схема B).
- **Пресет только с `config_source`.** Если парк настраивается с конфиг-сервера, в пресете
  достаточно блока `config_source` — `proxy` и `apps` дублировать не нужно. Служба
  установится, туннель подождёт, через ~10 секунд после старта приедет фид и применится.
  Пока фида нет, не проксируется ничего: это видно в панели, но снаружи выглядит как
  «ничего не работает», поэтому при ненадёжной связи с сервером оставьте в пресете рабочие
  `proxy` и `apps` — первый же фид их перекроет.
