...
(`managed_apps` / `managed_subnets` / `managed_domains`) и сбрасывает форсированные локи тумблеров
...
- **Прямые домены** — имена, которые у клиентов **никогда** не пойдут через прокси. Нужны,
  когда эндпоинт отказывает адресу прокси (анти-DDoS или гео-фильтр молча дропает SYN с
  хостингового IP) или когда внутренние узлы адресуются по имени. Запись с точкой в начале
  (`.corp.local`) — только поддомены, без точки — само имя и всё под ним.

  Пустое поле означает «ничего не навязываем»: ключ в фид не попадает, и собственный список
  клиента остаётся как был. Это отличается от приложений и подсетей, где пустое поле
  очищает вклад канала. Причина: пустой список доменов означал бы «не исключать ни одного
  имени» и снёс бы встроенные исключения клиента — mDNS и проверочные имена Windows, по
  которым система решает, есть ли вообще интернет.
